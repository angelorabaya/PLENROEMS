...
const PermitTypes = () => {
    ...
    const [permitTypeToDelete, setPermitTypeToDelete] = useState(null);
    ...
    const fetchPermitTypes = async () => {
        setLoading(true);
        try {
            const data = await api.getPermitTypes();
            setPermitTypes(data);
        } catch (err) {
    ...
    const handleSave = async (data) => {
        try {
            if (editingPermitType) {
                await api.updatePermitType(editingPermitType.tp_ctrlno, data);
                setSuccess('Permit type updated successfully');
            } else {
                await api.createPermitType(data);
                setSuccess('Permit type added successfully');
            }
            fetchPermitTypes();
    ...
    const handleDelete = async () => {
        if (!permitTypeToDelete) return;
        try {
            await api.deletePermitType(permitTypeToDelete.tp_ctrlno);
            setSuccess('Permit type deleted successfully');
            fetchPermitTypes();
            setIsDeleteModalOpen(false);
            setPermitTypeToDelete(null);
            setTimeout(() => setSuccess(''), 3000);
        } catch (err) {
            setError(err.message);
            setTimeout(() => setError(''), 3000);
    ...
    const columns = useMemo(
        () => [
            ...
            {
                accessorKey: 'tp_expyear',
                header: 'Expiration',
                size: 150,
                cell: ({ getValue }) => {
                    const months = getValue() || 0;
                    let displayText = '';
                    ...
                    if (months === 0) {
                        ...
                        displayText = `${years} ${years === 1 ? 'year' : 'years'} ${remainingMonths} ${remainingMonths === 1 ? 'month' : 'months'}`;
                    } else {
                        displayText = `${months} ${months === 1 ? 'month' : 'months'}`;
                    }

                    return <span className="cell-text">{displayText}</span>;
                },
            },
            {
                id: 'actions',
                    ...
                    <div className="actions-container">
                        ...
                        {isAdmin && (
                            <button
                                ...
                                onClick={() => {
                                    setPermitTypeToDelete(row.original);
    ...
    return (
        <div className="page-container">
            ...
            <div className="page-header">
                <h1 className="page-title">Permit Type Management</h1>
            ...
            {loading && (
                <div className="loading-container">
                    ...
                    <span className="loading-text">Loading permit types...</span>
                </div>
            )}
            ...
            <DeleteModal
                ...
                message="Are you sure you want to delete this permit type?"
            />
        </div>
    );
};
