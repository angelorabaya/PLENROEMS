...
const BarangayShareReport = () => {
    ...
    const loadReportData = async () => {
        ...
        setError(null);
        try {
            console.log(`Fetching barangay share for ${barangay}, ${municipality} - ${year}`);
            const result = await api.getBarangayShare(year, municipality, barangay);
    ...
    // Calculate total
    const totalAmount = data.reduce((sum, row) => sum + (Number(row.total_share) || 0), 0);
        ...
        <div className="barangay-share-report">
            ...
            <div className="barangay-print-toolbar">
                ...
                <div className="toolbar-meta">
                    Barangay Share Report — {barangay}, {municipality} ({year})
                </div>
            </div>
                ...
                {loading ? (
                    ...
                    <div className="barangay-report-paper">
                        ...
                        <section className="barangay-report-title">
                            <h1>BARANGAY 40% REVENUE SHARE REPORT (MONTHLY)</h1>
                        ...
                        <table className="barangay-table">
                            ...
                            <tbody>
                                {data.map((row, idx) => (
                                    <tr key={`${row.MonthName}-${idx}`}>
                                        <td className="month-cell">{row.MonthName}</td>
                                        <td className="amount-cell">
                                            {formatCurrency(row.total_share)}
                                        </td>
                                    </tr>
                                ))}
...
export default BarangayShareReport;
